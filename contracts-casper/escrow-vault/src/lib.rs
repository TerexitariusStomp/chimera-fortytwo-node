...
pub extern "C" fn create_job() {
    ...
    let valid_until = now + 3_600_000; // default 1 hour validity in milliseconds
    let nonce: u64 = 0;
    let task_type: u64 = 0;
    ...
    // Deposit to protocol fee recipient (escrow holder) since contract can't hold purse
    let protocol_fee_recipient: AccountHash = runtime::get_key(PROTOCOL_FEE_RECIPIENT)
        .unwrap_or_revert()
        .into_account()
        .unwrap_or_revert();
    system::transfer_to_account(protocol_fee_recipient, amount, None)
        .unwrap_or_revert();

    let mut pending: Vec<String> = read_dict(get_dict(PENDING_JOBS), "list").unwrap_or_default();
    pending.push(job_id.clone());
    write_dict(get_dict(PENDING_JOBS), "list", pending);

    let mut consumer_list: Vec<String> = read_dict(get_dict(CONSUMER_JOBS), &consumer.to_string())
        .unwrap_or_default();
    consumer_list.push(job_id.clone());
    write_dict(get_dict(CONSUMER_JOBS), &consumer.to_string(), consumer_list);
}
...
pub extern "C" fn provider_ack() {
    ...
    let mut provider_list: Vec<String> = read_dict(get_dict(PROVIDER_JOBS), &provider.to_string())
        .unwrap_or_default();
    provider_list.push(job_id.clone());
    write_dict(get_dict(PROVIDER_JOBS), &provider.to_string(), provider_list);
}
...
pub extern "C" fn claim_payment() {
    ...
    let _provider_payout = amount - protocol_fee;

    // Payout is recorded; actual transfer must be done by protocol fee recipient account
    write_dict(jobs_dict, &format!("{}:provider_claimed", job_id), true);
    write_dict(jobs_dict, &format!("{}:provider_payout", job_id), _provider_payout);
    ...
    if protocol_fee > U512::from(0) {
        let fees_uref: URef = runtime::get_key(PROTOCOL_FEES)
            ...
            .into_uref()
            .unwrap_or_revert();
        let current: U512 = storage::read(fees_uref).unwrap_or_revert().unwrap_or_default();
        storage::write(fees_uref, current + protocol_fee);
    }
}
...
pub extern "C" fn withdraw_protocol_fees() {
    require_owner();
    let _amount: U512 = runtime::get_named_arg("amount");
    let fees_uref: URef = runtime::get_key(PROTOCOL_FEES)
        .unwrap_or_revert()
        .into_uref()
        .unwrap_or_revert();
    ...
    // Actual withdrawal must be done by owner account manually
    // Contract records the obligation only
    storage::write(fees_uref, U512::from(0));
}
...
pub extern "C" fn refund_job() {
    ...
    let _amount: U512 = read_dict(jobs_dict, &format!("{}:amount", job_id)).unwrap_or_default();
    write_dict(jobs_dict, &format!("{}:state", job_id), STATE_REFUNDED);
}
...
pub extern "C" fn auto_release() {
    ...
    let state: u8 = read_dict(jobs_dict, &format!("{}:state", job_id))
        .unwrap_or(STATE_REFUNDED);
    if state != STATE_PENDING && state != STATE_ASSIGNED {
        runtime::revert(ApiError::User(4));
    }
